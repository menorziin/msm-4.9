				 ...
				 u32 scrambler_mode, u32 mono_mode)
{
	...
	if (format == ASM_MEDIA_FMT_AAC_V2) {
		uint32_t frame_size_ctl_value = config.port.enc_blk_param.enc_blk_config.
				aac_config.frame_ctl.ctl_value;
		if (frame_size_ctl_value > 0) {
			config.param.payload_size =
				payload_size + sizeof(config.port.frame_ctl_param);
			config.pdata.param_id =
				AFE_PARAM_ID_AAC_FRM_SIZE_CONTROL;
			config.pdata.param_size = sizeof(config.port.frame_ctl_param);
			config.port.frame_ctl_param.ctl_type = config.port.enc_blk_param.
				enc_blk_config.aac_config.frame_ctl.ctl_type;
			config.port.frame_ctl_param.ctl_value = frame_size_ctl_value;
			pr_debug("%s: sending AFE_PARAM_ID_AAC_FRM_SIZE_CONTROL to DSP\n",
				  __func__);
			ret = afe_apr_send_pkt(&config, &this_afe.wait[index]);
			if (ret) {
				pr_err("%s: AFE_PARAM_ID_AAC_FRM_SIZE_CONTROL for port 0x%x failed %d\n",
					__func__, port_id, ret);
				goto exit;
			}
		}
